...
import {
  AddressModificationModel, BaseListModel, CityModel, CountryModel,
  LovModel, SortFilterModel, StateModel, ZipCodeModel
}
  from '../../../models';
...
import { AddressModificatioPermanentService } from 'src/app/shared/services/transaction/data-entry/address-modification-permanent.service';
...
export class AddressModificationPermanentComponent
  ...
  constructor(private lovService: LOVService,
    ...
    private addressService: AddressModificatioPermanentService,
  ...
  initForm() {
    this.formInput = new FormGroup({
      // accountType: new FormControl('', [Validators.required, validateWhiteSpace,Validators.minLength(35), Validators.maxLength(35)]),
    //  add1: new FormControl('', [Validators.required, validateWhiteSpace, validateWhiteSpace, Validators.maxLength(36),
      //Validators.pattern(PatternValidation.CommaNotAllowed)]),
  ...
  // getData() {
  //   return this.addressService.getDataByDataEntryId(this.recId);
  // }
  ...
  isFormValid(isDestroyed = false) {
    this.frontendHelperService.emitFormValid(
      EmitType.AddressModificationPermanent,
  ...
  getFormValues(): AddressModificationModel {
    ...
    model.cmsAddressPermId = this.entity && this.entity.cmsAddressPermId > 0 ? this.entity?.cmsAddressPermId : 0;
  ...
  setLov(): Observable<BaseListModel<LovModel>>[] {
    ...
    //lovRequests.push(this.lovService.getLOVData(LOV.Inward));
