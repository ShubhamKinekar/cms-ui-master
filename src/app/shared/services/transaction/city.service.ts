...
export class CityService extends BaseDataService {
  ...
  getAllData<CityModel>(sortFilterData: SortFilterModel): Observable<BaseListModel<CityModel>> {
    const records: BaseListModel<CityModel> =
      new BaseListModel<CityModel>();
    return this.http
      .post('trans/city/getCity', sortFilterData)
      .pipe(
        map((result: any) => {
  ...
  ): Observable<BaseListModel<CityModel>> {
    const records: BaseListModel<CityModel> =
      new BaseListModel<CityModel>();
