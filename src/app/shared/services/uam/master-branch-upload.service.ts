...
export class MasterBranchUploadService extends BaseDataService {
  ...
  addData(request: any): Observable<number> {
    return this.http.get('master/branch/readMasterBranchCsvUpload/' + request);
  }
  ...
  uploadFile(request: FormData): Observable<string | null> {
    ...
    return this.http
      .post('master/branch/masterBranchCsvUpload', request, options)
      .pipe(
        map((result: any) => {
