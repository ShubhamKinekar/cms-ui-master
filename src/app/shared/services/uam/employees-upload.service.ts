...
export class EmployeesUploadService extends BaseDataService {
  ...
  addData(request: any): Observable<number> {
    return this.http.get('master/Employees/readEmployeeCsvUpload/' + request);
  }
  ...
  uploadFile(request: FormData): Observable<string | null> {
    ...
    return this.http
      .post('master/Employees/employeeCsvUpload', request, options)
      .pipe(
        map((result: any) => {
